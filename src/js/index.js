...
class IndexApp {
    ...
    constructor() {
        ...
        this.sqlQueryInput = new base.SqlQueryInput(document.querySelector("#sqlite-query-input"), document.querySelector("#sqlite-query-input-content"), document.querySelector("#sqlite-query-input-backdrop"), new base.AutoComplete(document.querySelector("#autocomplete-area"), document.querySelector("#sqlite-query-input"), document.querySelector("#complete-sql-query")));
        // initialize console
        this.consoleOutput = new base.ConsoleOutput(document.querySelector("#console-content"));
        // initialize settings menu
        this.settingsMenu = new base.SettingsMenu(document.querySelector("#header-settings"), document.querySelector("#settings-area"), document.querySelector("#settings-area-close-btn"));
        // initialize modal window
        this.safeModeModal = new base.SafeModeModal(document.querySelector("#confirm-safe-mode-alert"), document.querySelector("#confirm-safe-mode-alert-ok-btn"), document.querySelector("#confirm-safe-mode-alert-cancel-btn"), document.querySelector("#confirm-safe-mode-alert-close-btn"));
        // initialize worker
        ...
        this.copyTablePageMenu = new base.CopyTablePageMenu(document.querySelector("#copy-table-page-elem"), document.querySelector("#set-copy-table-page"), document.querySelector("#copy-table-page-close-btn"), document.querySelector("#remote-tables-names"), document.querySelector("#table-select-copy-mode"), document.querySelector("#input-remote-db-table-writes-count"), document.querySelector("#input-remote-db-table-source-table-name"), document.querySelector("#input-remote-db-table-local-db-path"), document.querySelector('#copy-table-page-elem button.send-btn'));
    ...
    clone() {
        ...
        this.sendToRemoteString(query_to_remote).then((data) => {
            ...
            this.sendToRemoteString(`SELECT * FROM sqlite_master WHERE name = '${table_name}'`)
                ...
                this.sqlWorker.sendCopyQuery(add_copied_data_query, table_name, local_db_path);
            });
        });
    }
    ...
    setupEventListeners() {
        ...
        this.copyTablePageMenu.close_btn.addEventListener("click", () => this.copyTablePageMenu.close());
            ...
            if (this.settings.readOnly_sett && this.settings.readOnly_sett.getStorageValue()) {
                ...
                this.safeModeModal.show(() => {
                    this.sendToRemote().then(() => {
                        setTimeout(() => {
                            if (isChangeDbStruct)
                                this.remoteSqlSender.updateRemoteTablesList();
                        }, 300);
                    });
                });
            }
    ...
    async sendToRemote() {
        ...
        if (response.success) {
            // show result
            this.consoleOutput.print(new base.ConsoleTableMessage([], new base.Table(response.result[0].results), true));
        }
        else {
            if (response.errors && response.errors[0].message === "empty_basic_db_link") {
                ...
                this.consoleOutput.print(new base.ConsoleTextMessage(["error-text"], response.errors[0].message, true));
            }
        }
    }
    async sendToRemoteString(query) {
        return await this.remoteSqlSender.sendQuery(query);
    }
    // shows access blocked warning
    showBlockAccessAlert() {
    ...
    setDbModeToggleView(targetState) {
        if (this.settings.isGlobal_sett) {
    ...
    setSafeModeToggleView(targetState) {
        if (this.settings.safeMode_sett) {
    ...
    setReadOnlyModeToggleView(targetState) {
        if (this.settings.readOnly_sett) {
